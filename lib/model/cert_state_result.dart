...
class CertStateResult {
  final CertStatus? status;
  final int? certId;
  final int? myRating;
  final String? myReview;

  const CertStateResult({this.status, this.certId, this.myRating, this.myReview});

  static const none = CertStateResult();

  factory CertStateResult.fromJson(Map<String, dynamic> json) {
    final state = json['certState'] as String?;
    if (state == null || state == 'NONE') return CertStateResult.none;
    return CertStateResult(
