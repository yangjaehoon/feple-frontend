...
void main() {
    ...
    await tester.pumpWidget(
      EasyLocalization(
        ...
        child: CustomThemeHolder(
          ...
          changeTheme: (_) {},
          child: MaterialApp(
            home: FestivalCalendar(artistId: 1, artistName: '아티스트'),
          ),
        ),
      ),
    );
  ...
  group('FestivalCalendar 렌더링', () {
    testWidgets('예정 일정과 지난 일정을 구분해서 보여준다', (tester) async {
  ...
  // 일정 카드 탭 시 이동하는 FestivalInformationFragment는 FestivalPoster/
  ...
  group('FestivalCalendar 일정 카드 탭', () {
  ...
  group('FestivalCalendar 달력 저장', () {
    testWidgets('날짜가 있으면 달력에 저장한다', (tester) async {
      when(() => mockScheduleService.fetchSchedule(1)).thenAnswer((_) async => [
            _schedule(title: '일정', startDate: '2026-08-01'),
          ]);
    ...
    testWidgets('날짜가 없으면 에러 스낵바를 보여주고 저장하지 않는다', (tester) async {
      when(() => mockScheduleService.fetchSchedule(1))
          .thenAnswer((_) async => [_schedule(title: '일정', startDate: null)]);
