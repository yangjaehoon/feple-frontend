...
WeatherModel _weather({String skyCode = '1', String ptyCode = '0'}) {
  return WeatherModel(
    fcstDate: '20260801',
    minTemp: 20,
    maxTemp: 28,
...
void main() {
  ...
  group('WeatherConditionIcon.conditionIcon', () {
    test('ptyCode가 강수 형태면 그것으로 결정된다 (skyCode 무시)', () {
      expect(_weather(ptyCode: '1', skyCode: '1').conditionIcon, '🌧');
      expect(_weather(ptyCode: '2', skyCode: '1').conditionIcon, '🌨');
      expect(_weather(ptyCode: '3', skyCode: '1').conditionIcon, '❄️');
      expect(_weather(ptyCode: '4', skyCode: '1').conditionIcon, '🌦');
    });

    test('ptyCode가 없으면(0) skyCode로 결정된다', () {
      expect(_weather(ptyCode: '0', skyCode: '3').conditionIcon, '🌤');
      expect(_weather(ptyCode: '0', skyCode: '4').conditionIcon, '☁️');
      expect(_weather(ptyCode: '0', skyCode: '1').conditionIcon, '☀️');
    });
  });
}
