...
class _FavoriteBoardsSectionState extends State<FavoriteBoardsSection> {
  ...
  Widget _buildBoardList(
    ...
    // 기준 390px: 리스트 높이 120(0.308) — 가로 ListView.builder라 폭(w) 기준 사용
    final listHeight = ResponsiveSize(context).w(120);
    return SizedBox(
      height: listHeight,
...
class _BoardTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final colors = context.appColors;
    // 기준 390px: 보드 카드 110(0.282) — 가로 ListView.builder라 폭(w) 기준 사용
    final cardSize = ResponsiveSize(context).w(110);
    return TapScale(
      onTap: () {
